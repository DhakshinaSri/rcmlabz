...
export default function Navbar() {
 ...
 const menus = [
...

  {
    ...
    subMenu: [
      {
        name: "Target Institutions & Programs",
        path: "/targetprograms",
      },
      {
  ...
  { name: "RCM NextGen Labz", path: "/labz" },
  { name: "Placement Assistance", path: "/placement" },
                      ...
                      <div className="flex flex-col mt-6 text-center space-y-6">
                        {item.subMenu.map((sub, i) => (
                          <Link
                            key={i}
                            to={sub.path}
                            className="text-[22px] font-medium text-gray-800 hover:text-[#348CD7]"
                            onClick={() => {
                              setIsOpen(false);
                              setOpenDropdown(null);
                            }}
